...
const OrderList: React.FC = React.memo(() => {
  ...
  const getStatusProps = (status: string) => {
    switch (status) {
      case "SUCCESS":
        return { statusText: "THÀNH CÔNG", tagColor: "green" };
      case "FAILED":
        return { statusText: "THẤT BẠI", tagColor: "red" };
      default:
        return { statusText: "UNKNOWN", tagColor: "grey" };
    }
  };
  ...
  const columns: TableProps<OrderInfo>["columns"] = [
    ...
    },
    {
      ...
      render: (status: string) => {
        const { statusText, tagColor } = getStatusProps(status);
